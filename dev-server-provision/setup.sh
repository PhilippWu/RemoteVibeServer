...
# In IP-only mode only CODER_ADMIN_PASSWORD is truly required.
# For domain-based deployments the full set of DNS/CF vars is validated.
IP_ONLY="${IP_ONLY:-false}"

if [[ "$IP_ONLY" == "true" ]]; then
  REQUIRED_VARS=(CODER_ADMIN_PASSWORD)
else
  REQUIRED_VARS=(DOMAIN SUBDOMAIN EMAIL)
fi
...
# Placeholder — will be overridden below if IP_ONLY=true
export FQDN
...
# In IP-only mode we override FQDN to the bare IP and set the Coder URL
# to plain HTTP so that Caddy and all downstream steps use the right value.
if [[ "$IP_ONLY" == "true" ]]; then
  FQDN="$PUBLIC_IP"
  export FQDN
  CODER_ACCESS_URL="http://$PUBLIC_IP"
  export CODER_ACCESS_URL
  log "IP-only mode: FQDN=$FQDN  CODER_ACCESS_URL=$CODER_ACCESS_URL"
else
  log "FQDN resolved to $FQDN"
  CODER_ACCESS_URL="https://$FQDN"
  export CODER_ACCESS_URL
fi
# ---------------------------------------------------------------------------
...
push_coder_template() {
  ...
  log "Creating first Coder admin user (${EMAIL:-admin@localhost}, username: ${admin_user}) …"
  local resp_body resp_code
  resp_body="$(mktemp)"
  resp_code="$(curl -s -o "$resp_body" -w '%{http_code}' \
    -X POST "${coder_api}/api/v2/users/first" \
    -H "Content-Type: application/json" \
    -d "{\"email\":\"${EMAIL:-admin@localhost}\",\"username\":\"${admin_user}\",\"password\":\"${admin_pass}\",\"trial\":false}" \
...
enable_coder_proxy() {
  ...
  if [[ "$IP_ONLY" == "true" ]]; then
    # IP-only: plain HTTP on the public IP, no TLS
    cat > "$caddyfile" <<CADDYEOF
# =============================================================================
# Caddyfile — IP-only mode (plain HTTP)
# auto-generated by RemoteVibeServer provisioning
# =============================================================================
...
http://$PUBLIC_IP {
    ...
    # Minimal security headers (no HSTS in HTTP-only mode)
    header {
...
CADDYEOF
  ...
  caddy validate --config "$caddyfile" --adapter caddyfile \
    ...
    && log "Coder is now accessible at $CODER_ACCESS_URL" \
...
if [[ -n "${_CODER_ADMIN_PASS:-}" ]]; then
  {
    ...
    echo "coder_admin_email=${EMAIL:-admin@localhost}"
...
log " Coder URL : $CODER_ACCESS_URL"
log " Status    : $STATUS_FILE"
if [[ -n "${_CODER_ADMIN_PASS:-}" ]]; then
  log " Admin     : ${EMAIL:-admin}  /  password in status file"
fi
log " Log       : $LOG_FILE"
log "============================================"
